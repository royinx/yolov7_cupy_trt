...
# def _pin_memory(array: np.ndarray):
#     device_ptr = cp.cuda.alloc_pinned_memory(array.nbytes)
#     # host_array = np.frombuffer(device_ptr, array.dtype, array.size).reshape(array.shape)
#     host_array = np.ndarray(buffer=device_ptr, dtype=array.dtype, shape = array.shape)
#     return host_array, device_ptr
...
# class TensorRT(RootClass):
    ...
    def allocate_buffers(self):
        ...
        for binding in self.engine:
            ...
            # _place_holder = np.empty(size,dtype).nbytes

            # host_mem = cp.empty_pinned(size, dtype)
            # device_mem = cp.cuda.alloc(host_mem.nbytes)

            # device_mem = cp.cuda.alloc_pinned_memory(_place_holder)
            # device_array = cp.ndarray(shape, dtype, memptr=device_mem)
            # host_mem = np.frombuffer(device_mem, dtype, size)
            
            device_array = cp.empty(shape, dtype)
            # host_array = np.frombuffer(device_array.data, dtype, size).reshape(shape)
            # host_mem, device_ptr = _pin_memory(np.empty(size,dtype))
# host_array, device_ptr

            self.bindings.append(int(device_array.data)) # cupy array ptr
            # print(bindings)
            # Append to the appropriate list.
            if self.engine.binding_is_input(binding):
                # self.inputs.append(HostDeviceMem(host_array, device_array))
    ...
    def inference(self,inputs:np.ndarray) -> list: # input: <NCHW>
        ...
        # [inp.device.set(inp.host) for inp in self.inputs]
        ...
        # [out.device.get(out=out.host) for out in self.outputs]

        # for rs in [out.device for out in self.outputs]:
        #     print(rs.shape)
        # exit()
